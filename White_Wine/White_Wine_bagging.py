...
l_maxsamples = np.array([1500,2000,2500,3000]) # List with max samples drawn to train each decision tree
values = np.zeros((len(l_estimators),len(l_maxsamples))) # Making matrix to store the accuracy scores
confusion_matrices = np.zeros((len(l_estimators),len(l_maxsamples),7,7)) # Making matrix to the confusion matrices
